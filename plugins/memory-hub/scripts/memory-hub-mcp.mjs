...
server.registerTool(
  "memory_supersede",
  {
    ...
    inputSchema: z.object({
      ...
      kind: z.string().default("fact"),
      sourceAgent: z.string().default(DEFAULT_AGENT),
      reason: z.string().default("Superseded by a corrected or newer memory"),
    }),
  },
  async ({ previousCategory, previousId, content, category, kind, sourceAgent, reason }) =>
    textResult(
...
if (config().adminToken) {
  server.registerTool(
    "memory_purge",
    {
      description: "Permanently delete a memory. Admin token required.",
      inputSchema: z.object({
        category: z.string(),
        memoryId: z.string(),
      }),
