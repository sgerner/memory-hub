...
function shouldStoreTurnSummary(prompt, assistantSummary) {
  const promptText = String(prompt || '').trim();
  const assistantText = String(assistantSummary || '').trim();
...
async function stop(input) {
  ...
  try {
    if (shouldStoreTurnSummary(prompt, assistantSummary)) {
      ...
      debugLog('Stop skipped transient summary', {
        ...
        assistant: clip(assistantSummary, 240),
      });
    }
  } catch (error) {
